  ...
  while (current.className != "treeroot") {
    ...
    for (i = 0; i < parent.childNodes.length; i++) {
      ...
      if (after && child.id && child.id.indexOf("tn") > -1) {
        ...
        if (ann)
          ann.style.top = Number(ann.style.top.replace("px", "")) + delta;
      }
...
function GetExpandCollapseDelta(nodeId, childContainerId) {
  ...
  var lastNodeBottom =
    lastNodeTop + Number(lastNode.style.height.replace("px", ""));
...
function GetLastVisibleChild(containerId) {
  ...
  var subContainer = document.getElementById(
    "cnc" + lastNodeId.replace("tn", "")
  );
...
function InitializeTreeNode(nodeId, plusminusid, childContainerId, selectText) {
  ...
  s =
    s + "var container = document.getElementById('" + childContainerId + "');";
  s =
    s +
    "if (container.style.visibility != 'hidden') CollapseNode('" +
    nodeId +
    "', '" +
    childContainerId +
    "', '" +
  ...
  if ((index = navigator.userAgent.indexOf("MSIE")) >= 0)
    s =
      ...
      "document.getElementById('" +
      plusminusid +
      '\').attachEvent("onclick", ExpandCollapse' +
      plusminusid +
      ");";
  else
    s =
